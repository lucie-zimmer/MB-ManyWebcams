...
sampling_rate_data <- sampling_rate_data %>% rename(t = "t...0...8000.")
...
```{r}
# prepare trial by trial data

data_trial_by_trial <- bind_rows(
  # in lab data 
  data_pilot %>%
  filter(experiment_num=="pilot_1a" & t >= -4000 & t <= 0) %>%
  group_by(subid_new, trial_num) %>%
  summarize(lookingscore = sum(grepl("target", aoi, fixed = TRUE), na.rm = TRUE) / sum(grepl("target", aoi, fixed = TRUE) | grepl( "distractor", aoi, fixed = TRUE)))%>%
    na.omit()%>%
    rename(subid = subid_new),
  
# webcam data
sampling_rate_data %>%
  filter(t <= 4000) %>%
  mutate(subid = str_remove(subid, "_B"), 
         subid = str_remove(subid, "_A"))%>%
  filter(subid %in% data_webcam$subid)%>%
  group_by(subid, trial_num)%>%
  summarize(lookingscore = sum(grepl("target", aoi, fixed = TRUE), na.rm = TRUE) / sum(grepl("target", aoi, fixed = TRUE) | grepl( "distractor", aoi, fixed = TRUE)))

)%>%
  left_join(data%>%select(-percorr), by = "subid")%>%
  mutate(lookingscore = ifelse(lookingscore == 0, 0.000001, lookingscore),
         lookingscore = ifelse(lookingscore == 1, 0.999999, lookingscore))%>%
...
## On the difference in aggregation between the preprocessing script and the trial by trial data
```{r, warning=FALSE}

# The aggregated trial-by-trial data and the relative data lead to slightly different looking scores, as there is no resampling:
# In relative data, each trials looking score is automatically weighted on how many tracking gazepoints were generated during that trial (as all gazepoints across trials are averaged)
# In the tbt aggregate, each trials looking score is considered equally (as the average of every trial is taken, and then a average across the 4 trials is calculated) 
compare_relative_tbt <- data_trial_by_trial %>% group_by(subid) %>% summarise(ls_tbt_mean = mean(lookingscore)) %>% inner_join(data_webcam) %>% select(subid, ls_relative = percorr, ls_tbt_mean)


# port of the relevant part of the python preprocessing script to illustrate how the aggregation is done there

python_agg <- sampling_rate_data %>%
  filter(t <= 4000) %>%
  group_by(subid, aoi) %>%
  summarize(points = n()) %>% 
  mutate(freqR = points/sum(points)) %>%
  inner_join(
    read.csv(FILE_NAME_WEBCAM, stringsAsFactors = TRUE) %>% 
      mutate(subid = str_remove(subid, "_B"), 
             subid = str_remove(subid, "_A")
             )
    , by=c('subid', 'aoi')) %>% 
  select(subid, aoi, freqR, freq)

```


## Looking at spread of samplingrate during critical timeframe
```{r, warning=FALSE}

sampling_spread <- sampling_rate_data %>%
  filter(t <= 4000) %>% 
  select(subid,stimulus,t) %>% 
  group_by(subid, stimulus) %>% 
  mutate(delta = t - lag(t, default = 0)) %>% 
  mutate(sampling_hz = 1000.0/delta) %>%
  slice(2:n()) %>% 
  summarize(mean = mean(sampling_hz), sd=sd(sampling_hz))

```

## Differences between in-lab and webcam 

```{r, warning=FALSE}

h2.rand.model.beta.trial <- glmmTMB(lookingscore ~  method + z_trial_num + (1|lab) + (z_trial_num|subid), data = data_trial_by_trial, family = beta_family())

summary(h2.rand.model.beta.trial)
```
...
dataErrorbar <- group_by(data, method) %>%
  summarise(mean = mean(percorr), upper = mean + 1.96*(sd(percorr)/sqrt(n())), lower = mean - 1.96*(sd(percorr)/sqrt(n())))
...
## Package to do bootstrap
library(tidyboot)

## running this takes a minute or two
dataBootstrap <- data%>%
  group_by(method)%>%
  tidyboot_mean(col = percorr)
```
...
H2.plot <- ggplot(data, aes(x = method, y = percorr, colour = method, fill = method)) +
 ...
 geom_point(aes(x = method, y = mean), dataErrorbar, inherit.aes = FALSE, size = 2, color = "black") +
  geom_errorbar(aes(x = c("inlab", "webcam"), ymax = `upper`, ymin = `lower`), dataErrorbar, inherit.aes = FALSE, stat = "identity", width = 0.05, color = "black") +
  ylim(0, 1) +
  scale_colour_manual(values=c("skyblue", "red"))  +
  scale_fill_manual(values=c("skyblue",  "red")) +
  geom_hline(yintercept=0.5, linetype="dashed", color = "black") +
  labs(title="Proportional looking score per method (95% CIs)", x="Method", y="Proportional looking score") +
  scale_x_discrete(labels=c("lab" = "in-lab", "online" = "remote-webbased")) +
  theme_classic()
...
```{r, warning=FALSE}
data_trial_by_trial_webcam <- data_trial_by_trial%>%
  filter(method == "webcam")%>%
  left_join(data_webcam%>%select(subid, sampling_rate))%>%
  ungroup()%>%
  mutate(z_sampling_rate = scale(sampling_rate), 
         z_trial_num = scale(trial_num))

h2.rand.model.beta.trial.rate <- glmmTMB(lookingscore ~  z_sampling_rate + z_trial_num + z_age + (z_age + z_sampling_rate|lab) + (z_trial_num|subid), data = data_trial_by_trial_webcam, family = beta_family())

summary(h2.rand.model.beta.trial.rate)
```

## Effect of age: adding age as a predictor
