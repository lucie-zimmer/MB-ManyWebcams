...
for (i in 1:nrow(data_inlab)) {
  s = data_inlab$subid_new[i]
  data_inlab$age_days[i] <-unique(data_pilot[data_pilot$subid_new == s, "age"])
}

data_inlab <- data_inlab %>%
  mutate(subid = subid_new) %>%
  select(subid, lookingscore) 
...
for (i in 1:nrow(data_webcamDemo)) {
  if(!is.na(data_webcamDemo$gender[i])){
    if(data_webcamDemo$gender[i] == "girl"){
      data_webcamDemo$gender[i] = "girl"
    } else if(data_webcamDemo$gender[i] == "boy"){
...
data_webcamDemo$z_age <- scale(data_webcamDemo$age, scale = TRUE)
...
# is the 8 correct?
exclusion.table <- matrix(c(nIncludedWebcam, nrow(data_inlab), nTestedWebcam - nIncludedWebcam, 8), nrow = 2, ncol = 2, byrow = T)
...
```{r}
ex.model <- lm(percorr ~ z_age, data = data_webcamDemo)
