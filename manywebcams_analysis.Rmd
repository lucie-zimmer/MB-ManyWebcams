...
if(dir.exists(DIR_NAME_DATA_MERGE)){
  ...
  for(lab in labs){
    ...
    data_demo_part <- read.csv2(file.path(DIR_NAME_DATA_MERGE, lab, FILE_NAME_DEMO), stringsAsFactors = TRUE)
    data_demo_part$data_source <- lab
    data_demo_list <- append(data_demo_list, list(data_demo_part))
    
    manual_exclusions_part <- read.csv2(file.path(DIR_NAME_DATA_MERGE, lab, FILE_NAME_EXCLUSION_MANUAL), stringsAsFactors = TRUE)
    ...
    if (file.exists(f_exclusion_sampling)){
      ...
      if(samplingrate_exclusions_string != "[]"){
        samplingrate_exclusions_string <- substr(samplingrate_exclusions_string, 3, nchar(samplingrate_exclusions_string)-3)
  ...
  rm(data_webcam_part, data_demo_part, manual_exclusions_part)
