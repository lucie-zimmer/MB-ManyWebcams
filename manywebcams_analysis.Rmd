...
data_trial_by_trial_webcam <- data_trial_by_trial%>%
  ...
  left_join(data_webcam)%>%
  ungroup()%>%
  mutate(z_sampling_rate = scale(sampling_rate), 
         z_trial_num = scale(trial_num))

h2.rand.model.beta.trial.rate <- glmmTMB(lookingscore ~  z_sampling_rate + z_trial_num +(z_sampling_rate|lab) + (z_trial_num|subid), data = data_trial_by_trial_webcam, family = beta_family())
