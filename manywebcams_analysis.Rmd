...
# remove column introduced during merge
data_webcam <- data_webcam[, c("subid", "percorr")]
...
data_webcam$subid[which(data_webcam$subid == "UBC002")] = "UBC_002"
data_webcam$subid[which(data_webcam$subid == "UBC007")] = "UBC_007"
data_webcam$subid[which(data_webcam$subid == "UBC011")] = "UBC_011"
data_webcam$subid[which(data_webcam$subid == "UBC014")] = "UBC_014"
data_webcam$subid[which(data_webcam$subid == "UBC015")] = "UBC_015"
data_webcam$subid[which(data_webcam$subid == "UBC017")] = "UBC_017"

x = which(data_webcam$subid %in% data_webcamDemo_temp$subid == FALSE)
x_subid = data_webcam$subid[x]
rm(data_webcamDemo_temp)

# MPI_003 not in data_demo!
...
# adjust gender entries
for (i in 1:nrow(data_webcamDemo)) {
  if(!is.na(data_webcamDemo$gender[i])){
    if(data_webcamDemo$gender[i] == "girl" | data_webcamDemo$gender[i] == "F"){
      data_webcamDemo$gender[i] = "girl"
    } else if(data_webcamDemo$gender[i] == "boy" | data_webcamDemo$gender[i] == "M"){
      data_webcamDemo$gender[i] = "boy"
    }
  }
}
